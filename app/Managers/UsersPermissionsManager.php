...
class UsersPermissionsManager extends DatabaseManager
{
    ...
    /**
     * Récupère une seule instance de UsersPermissions avec une recherche signée dans la base de données.
     * 
     * Cette méthode est utilisée pour effectuer une recherche dans la table users_permissions avec une logique spécifique liée aux "signatures" (signées).
     * Elle retourne une seule instance de UsersPermissions ou null si aucune correspondance n'est trouvée.
     * 
     * @param array $param_Data Données à utiliser pour filtrer la recherche.
     * @param array $param_Parameters Paramètres supplémentaires pour la requête.
     * @return UsersPermissions|null Retourne une instance de UsersPermissions ou null si aucune correspondance.
     */
    public function findOneSignUsersPermissions(array $param_Data = [], array $param_Parameters = []) : ?UsersPermissions
    {
        // Appel à la méthode findOne de DatabaseManager avec les paramètres nécessaires pour rechercher une seule entrée dans la table
        return parent::findOneSign('users_permissions', UsersPermissions::class, $param_Data, $param_Parameters);
    }
