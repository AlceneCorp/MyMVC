...
class UsersQuizManager extends DatabaseManager
{
    ...
    /**
     * Récupère une seule instance de UsersQuiz avec une recherche signée dans la base de données.
     * 
     * Cette méthode est utilisée pour effectuer une recherche dans la table users_quiz avec une logique spécifique liée aux "signatures" (signées).
     * Elle retourne une seule instance de UsersQuiz ou null si aucune correspondance n'est trouvée.
     * 
     * @param array $param_Data Données à utiliser pour filtrer la recherche.
     * @param array $param_Parameters Paramètres supplémentaires pour la requête.
     * @return UsersQuiz|null Retourne une instance de UsersQuiz ou null si aucune correspondance.
     */
    public function findOneSignUsersQuiz(array $param_Data = [], array $param_Parameters = []) : ?UsersQuiz
    {
        // Appel à la méthode findOne de DatabaseManager avec les paramètres nécessaires pour rechercher une seule entrée dans la table
        return parent::findOneSign('users_quiz', UsersQuiz::class, $param_Data, $param_Parameters);
    }
