...
/**
 * Class QuizManager
 * Cette classe gère les opérations liées à la manipulation des instances de Quiz dans la base de données.
 */
class QuizManager extends DatabaseManager
{
    /**
     * Récupère une seule instance de Quiz à partir de la base de données.
     * 
     ...
     */
    public function findOneQuiz(array $param_Data = [], array $param_Parameters = []) : ?Quiz
    {
     ...
     * @param array $param_Data Données à utiliser pour filtrer la recherche.
     * @param array $param_Parameters Paramètres supplémentaires pour la requête.
     * @return array Tableau d'instances de Quiz.
     */
    public function findAllQuiz(array $param_Data = [], array $param_Parameters = []) : array
    {
        // Appel à la méthode findAll de DatabaseManager pour récupérer toutes les entrées dans la table
        return parent::findAll('quiz', Quiz::class, $param_Data, $param_Parameters);
    }

    /**
     * Ajoute une nouvelle instance de Quiz dans la base de données.
     * 
     * @param array $param_Data Données à insérer dans la table.
     * @return int Retourne l'ID de l'élément inséré.
     */
    public function addQuiz(array $param_Data) : int
    {
        // Appel à la méthode insert de DatabaseManager pour insérer de nouvelles données dans la table
        return parent::insert('quiz', $param_Data);
    }

    /**
     * Met à jour une instance de Quiz dans la base de données.
     * 
     ...
     */
    public function updateQuiz(array $param_Data, $param_id) : int
    {
        ...
        return parent::update('quiz', $param_Data, $param_id);
    }

    /**
     * Supprime une instance de Quiz dans la base de données.
     * 
     * @param array $param_Data Données permettant de trouver l'élément à supprimer.
     * @return bool Retourne true si la suppression a réussi, sinon false.
     */
    public function deleteQuiz(array $param_Data) : bool
    {
        // Appel à la méthode delete de DatabaseManager pour supprimer des données de la table
        return parent::delete('quiz', $param_Data);
    }

    /**
     * Compte le nombre d'enregistrements de Quiz dans la base de données en fonction des paramètres.
     * 
     ...
     */
    public function countQuiz(array $param_Data = [], array $param_Parameters = []) : int
    {
        return parent::count('quiz', Quiz::class, $param_Data, $param_Parameters);
    }

    /**
     * Vide (TRUNCATE) la table quiz.
     * 
     * @return bool Retourne true si l'opération a réussi, sinon false.
     */
    public function truncateQuiz() : bool
    {
        // Appel à la méthode truncate de DatabaseManager pour supprimer toutes les lignes de la table
        return parent::truncate('quiz');
    }
}
