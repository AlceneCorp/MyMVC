...
use App\Modules\Questorium\Models\Subanswers; // Importation de la classe Subanswers, représentant un modèle d'entité de la base de données
...
class SubanswersManager extends DatabaseManager
{
    ...
    /**
     * Récupère une seule instance de Subanswers avec une recherche signée dans la base de données.
     * 
     * Cette méthode est utilisée pour effectuer une recherche dans la table subanswers avec une logique spécifique liée aux "signatures" (signées).
     * Elle retourne une seule instance de Subanswers ou null si aucune correspondance n'est trouvée.
     * 
     * @param array $param_Data Données à utiliser pour filtrer la recherche.
     * @param array $param_Parameters Paramètres supplémentaires pour la requête.
     * @return Subanswers|null Retourne une instance de Subanswers ou null si aucune correspondance.
     */
    public function findOneSignSubanswers(array $param_Data = [], array $param_Parameters = []) : ?Subanswers
    {
        // Appel à la méthode findOne de DatabaseManager avec les paramètres nécessaires pour rechercher une seule entrée dans la table
        return parent::findOneSign('subanswers', Subanswers::class, $param_Data, $param_Parameters);
    }
