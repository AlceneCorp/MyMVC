...
class {{ObjectName}}Manager extends DatabaseManager
{
    ...
    /**
     * Récupère une seule instance de {{ObjectName}} avec une recherche signée dans la base de données.
     * 
     * Cette méthode est utilisée pour effectuer une recherche dans la table {{TableName}} avec une logique spécifique liée aux "signatures" (signées).
     * Elle retourne une seule instance de {{ObjectName}} ou null si aucune correspondance n'est trouvée.
     * 
     * @param array $param_Data Données à utiliser pour filtrer la recherche.
     * @param array $param_Parameters Paramètres supplémentaires pour la requête.
     * @return {{ObjectName}}|null Retourne une instance de {{ObjectName}} ou null si aucune correspondance.
     */
    public function findOneSign{{ObjectName}}(array $param_Data = [], array $param_Parameters = []) : ?{{ObjectName}}
    {
        // Appel à la méthode findOne de DatabaseManager avec les paramètres nécessaires pour rechercher une seule entrée dans la table
        return parent::findOneSign('{{TableName}}', {{ObjectName}}::class, $param_Data, $param_Parameters);
    }
